...
struct TCompressBlocksState: public TBlockState {
    ...
    NYql::NUdf::TCounter CounterPeakBytes_;
...
public:
    TCompressBlocksFlowWrapper(TComputationMutables& mutables, IComputationWideFlowNode* flow, ui32 bitmapIndex, TVector<TBlockType*>&& types, TOperatorId operatorId)
    ...
    EFetchResult DoCalculate(NUdf::TUnboxedValue& state, TComputationContext& ctx, NUdf::TUnboxedValue*const* output) const {
        ...
        if (!s.Count) {
            do if (!s.InputSize_) {
                s.ClearValues();
                EFetchResult fetchResult;
                {
                    auto operatorUnguard = TOperatorUnguard();
                    fetchResult = Flow_->FetchValues(ctx, fields);
                }
...
public:
    TCompressStreamWrapper(TComputationMutables& mutables, IComputationNode* stream, ui32 bitmapIndex, ui32 inputWidth, TVector<TBlockType*>&& types, TOperatorId operatorId)
...
private:
    ...
    void InitializeBytesCounter(TComputationContext& ctx) const {
        if (ctx.CountersProvider) {
            TString id = TString(Operator_Filter) + "0";
            CounterPeakBytes_ = ctx.CountersProvider->GetCounter(id, Counter_PeakBytes, false);
        }
    ...
    const TVector<TBlockType*> Types_;
    const size_t WideFieldsIndex_;
    mutable TMaybe<NYql::NUdf::TCounter> CounterPeakBytes_;
};

IComputationNode* WrapCompressWithScalarBitmap(TComputationMutables& mutables, IComputationNode* compressArg, ui32 bitmapIndex, ui32 inputWidth, TVector<TBlockType*>&& types, TOperatorId operatorId) {
    if (auto* flowCompressArg = dynamic_cast<IComputationWideFlowNode*>(compressArg)) {
        ...
        return new TCompressStreamWrapper<ECompressType::BitmapIsScalar>(mutables, compressArg, bitmapIndex, inputWidth, std::move(types), operatorId);
    }
}

IComputationNode* WrapCompressScalars(TComputationMutables& mutables, IComputationNode* compressArg, ui32 bitmapIndex, ui32 inputWidth, TVector<TBlockType*>&& types, TOperatorId operatorId) {
    if (auto* flowCompressArg = dynamic_cast<IComputationWideFlowNode*>(compressArg)) {
        return new TCompressScalarsFlowWrapper(mutables, flowCompressArg, bitmapIndex, inputWidth);
    } else {
        return new TCompressStreamWrapper<ECompressType::AllScalars>(mutables, compressArg, bitmapIndex, inputWidth, std::move(types), operatorId);
    }
}

IComputationNode* WrapCompressBlocks(TComputationMutables& mutables, IComputationNode* compressArg, ui32 bitmapIndex, ui32 inputWidth, TVector<TBlockType*>&& types, TOperatorId operatorId) {
    if (auto* flowCompressArg = dynamic_cast<IComputationWideFlowNode*>(compressArg)) {
        return new TCompressBlocksFlowWrapper(mutables, flowCompressArg, bitmapIndex, std::move(types), operatorId);
    } else {
        return new TCompressStreamWrapper<ECompressType::Blocks>(mutables, compressArg, bitmapIndex, inputWidth, std::move(types), operatorId);
    }
}
...
IComputationNode* WrapBlockCompress(TCallable& callable, const TComputationNodeFactoryContext& ctx) {
    ...
    USER_LOG("Created Filter operator in Wrapper " << operatorId);
    ...
    if (bitmapIsScalar) {
        ...
        return WrapCompressScalars(ctx.Mutables, compressArg, bitmapIndex, inputWidth, std::move(types), operatorId);
    }
    return WrapCompressBlocks(ctx.Mutables, compressArg, bitmapIndex, inputWidth, std::move(types), operatorId);
}
